...
public final class SessionTest extends InstrumentationTestCase {
  ...
  public void singleLapSession() throws Exception {
    InputStream is = getInstrumentation().getTargetContext().getResources().getAssets().open("/single_lap_session.csv");
  ...
  public void multiLapSession() throws Exception {
    InputStream is = getInstrumentation().getTargetContext().getResources().getAssets().open("/multi_lap_session.csv");
    BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
    String line;
    SessionManager.getInstance().startSession(track);
    double startTime = System.currentTimeMillis() / 1000;
